...
def save_model_weights(model):
   """
   Takes in a model and saves the weights of said model to the directory "model_ckpts"
...
def download_prep_data():
   """
   If not already found, creates directories for annotations and images datasets.
   Shuffles them.
 
   Params: None
 
   Returns:
   image_path            -- path to images dataset
   image_path_to_caption -- path to captions for each image
   """
   ...
   return image_path, image_path_to_caption
 
 
def build_set_data(image_path, image_path_to_caption, BATCH_SIZE, num_images=10_000):
   """
   Builds appropriate, manageable datasets.
 
   Params:
   image_path            -- array of paths to images
   image_path_to_caption -- array of paths to captions for images
 
   Returns:
   image_dataset         -- correctly ordered and shuffled dataset of images
   caption_dataset       -- correctly ordered and shuffled dataset of captions
   image_name_vector     -- image names and captions
   """
   train_image_paths = image_path[:num_images]
   ...
   return image_dataset, caption_dataset, img_name_vector
 
def standardize(inputs):
   """
   Turns words to lowercase and removes punctuation from inputs.
   """
...
def main():
   """
   Main call function for the model. Creates the model with all proper variables
   proceeds to train the model, and then save model weights for later use.
   ...
   image_path, image_path_to_caption = download_prep_data()
   image_dataset, caption_dataset, img_name_vector = build_set_data(
       image_path,
